...
function DoctorLogin() {
  ...
  const loginHandler=async(e)=>{
    ...
    setErrror('');
    try{
        const loginresponse=await axios.post(`${import.meta.env.VITE_BASE_URL}/doctor/login`,{email,password},{withCredentials:true});
        if(loginresponse.status===200){

            const profileResponse=await axios.get(`${import.meta.env.VITE_BASE_URL}/doctor/profile`,{withCredentials:true});
